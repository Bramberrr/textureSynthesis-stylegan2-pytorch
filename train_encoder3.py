...
from model import GradualStyleEncoder
...
def train(args, loader, generator, encoder, e_optim, device, vgg, random_phase):
    ...
    for idx in pbar:
        ...
        # latent = encoder(real_img)
        gene_img, _ = generator([latent], phase_noise=phase_noise)
        list_activations_example = vgg(gene_img)
        ...
        loss = args.latent_weight * latent_loss + args.image_weight * image_loss

        # loss = args.latent_weight * latent_loss
        encoder.zero_grad()
        loss.backward()
        e_optim.step()
        ...
        if get_rank() == 0:
            pbar.set_description(f"latent: {latent_loss_val:4.4f}; image: {image_loss_val:4.4f}")
            # pbar.set_description(f"latent: {latent_loss_val:4.4f}")
            ...
            if i % args.save_ckpt_every == 0:
                torch.save({"e": e_module.state_dict(), "e_optim": e_optim.state_dict(), "args": args}, f"{args.output}/last.pt")
...
if __name__ == "__main__":
    ...
    encoder = GradualStyleEncoder(50)
    encoder.to(device)
